...
public class UISaveCorporationPointerInputAction extends Action {
	public ActionForward execute(ActionMapping actionMapping,
		...
		String[] AmmeterStyle = httpServletRequest
				.getParameterValues("useStyle");
		String[] LineLossQuantity = httpServletRequest
				.getParameterValues("LineLossQuantity");
		String[] TransLossQuantity = httpServletRequest
				.getParameterValues("TransLossQuantity");
		...
		/*
		 * String[] ExcepQuantity=
		 * httpServletRequest.getParameterValues("ExcepQuantity");
		 */
		...
		String changeLastWorkNum = httpServletRequest
				.getParameter("changeLastWorkNum");
		String changeThisWorkNum = httpServletRequest
				.getParameter("changeThisWorkNum");
		String changeLastIdleNum = httpServletRequest
				.getParameter("changeLastIdleNum");
		String changeThisIdleNum = httpServletRequest
				.getParameter("changeThisIdleNum");
		String changeworkQuantity = httpServletRequest
				.getParameter("changeworkQuantity");
		String changeunworkQuantity = httpServletRequest
				.getParameter("changeunworkQuantity");
		...
		// 添加停产，容量变化 modify by qiaoyouliang 2009-05-22 begin
		// 变化后的容量电量
		String rlquantityaf = httpServletRequest.getParameter("rongliangAf");

		// 容量电量变化后是否发生容量电价的变化
		String chgpriceafflag = httpServletRequest
				.getParameter("chgpriceafflag");

		// 容量电量变化后是否发生容量电量的变化
		String rlprice = httpServletRequest.getParameter("rlprice");

		// 添加停产，容量变化 modify by qiaoyouliang 2009-05-22 end
		...
		String conditionone = "1=1 and UserNo='" + userNo + "'"
				+ " and AMMETERSTYLE=0";
		Collection colone = blLwNewFactoryIndicatorFacade
				.findByConditions(conditionone);
		...
		for (int i = 0; i < ammeterNo.length; i++) {
			...
			lwNewFactoryIndicatorDto.setChgAmmeterQuantity(Double
					.parseDouble(ChgAmmeterQuantity[i]));
			lwNewFactoryIndicatorDto.setAmmeterStyle(AmmeterStyle[i]);
			lwNewFactoryIndicatorDto.setChangeBefore(state);
			
		    if(RongliangQuantity==null){
		    	lwNewFactoryIndicatorDto.setRlquantityafbefore(0);
		    }else{
		    	lwNewFactoryIndicatorDto.setRlquantityafbefore(Double.parseDouble(RongliangQuantity));
		    }
			...
			if (AmmeterStyle[i].equals("0")
					&& lwCoporationUserInfoDto.getIndustryType().equals("2")) {
				lwNewFactoryIndicatorDto.setNeedQuantity(Double
			...
			if (stopProduce.equals("1")) {
				...
				lwNewFactoryIndicatorDto.setChangeWorkPointer(Double
						.parseDouble(changeLastWorkNum));
				lwNewFactoryIndicatorDto.setChangeunWorkPointer(Double
						.parseDouble(changeLastIdleNum));
				lwNewFactoryIndicatorDto
						.setChangeworkQuantity(changeworkQuantity);
				lwNewFactoryIndicatorDto
						.setChangeUnworkQuantity(changeunworkQuantity);
				lwNewFactoryIndicatorDto
						.setChageChgAmmeterQuantity(changeChgAmmeterQuantity);
				lwNewFactoryIndicatorDto
						.setChangeunChgAmmeterQuantity(changeUnChgAmmeterQuantity);
				lwNewFactoryIndicatorDto
						.setChangeCompensateQuantity(changeCompensateQuantity);
				lwNewFactoryIndicatorDto
						.setChangeunCompensateQuantity(changeUnCompensateQuantity);
				// 添加停产，容量变化 modify by qiaoyouliang 2009-05-22 begin
				// lwNewFactoryIndicatorDto.setRlpriceaf(Double.parseDouble(rlpriceaf));
				lwNewFactoryIndicatorDto.setRlquantityaf(Double
						.parseDouble(rlquantityaf));
				lwNewFactoryIndicatorDto.setChgpriceafflag(chgpriceafflag);
				}else{
					lwNewFactoryIndicatorDto.setThisWorkNum(Double
							.parseDouble(ThisWorkNum[i]));
					lwNewFactoryIndicatorDto.setThisIdleNum(Double
			...
			lwNewFactoryIndicatorDto.setChgAmmeterQuantity(Double
					.parseDouble(ChgAmmeterQuantity[i]));
			lwNewFactoryIndicatorDto.setCompensateQuantity(Double
					.parseDouble(CompensateQuantity[i]));
			lwNewFactoryIndicatorDto.setStatMonth(inputdate);

			// lwWholeSaleIndicatorDto.setInputDate(new
			// Date().toString().substring(0, arg1));

			String conditiona = "flag=1 and  userNo='" + userNo + "'"
		...
		httpServletRequest.setAttribute("lwNewFactoryIndicatorDto2",
				lwNewFactoryIndicatorDto2);
